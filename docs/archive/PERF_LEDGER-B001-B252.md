...
> **Archived B001–B252 experiment ledger.** Measurements and refutations remain
> valid only for the commits and setups they name. Use the current root
> [`PERF_ROADMAP.md`](../../PERF_ROADMAP.md) for live priorities.
...
>
> Last re-measured **2026-08-30**. Every number below was measured on this repo;
...
## Current experiment registry — 2026-08-30
...
|---|---|---|
| **B252 LANDED — merge monotonic recycled-object address runs** | **hostile allocation-survival −2.93% [−3.34%, −0.28%]; shapes and normal suites neutral** | B252 LANDED (2026-08-30, `f5c33df`). Minor and major sweeps already append newly dead object shells in ascending heap-index order, but recycling re-sorted the whole retained pool by backing-address after every collection. The pool now proves an ascending prefix and monotonic suffix, then joins those two runs with a reverse/rotate merge; every uncertain or overlapping layout takes the unchanged full-sort fallback, preserving exact ascending-address allocation order. `ZIPP_NO_OBJ_POOL_RUN_SORT=1` restores the old route, and `ZIPP_GCSTATS=1` proves run-merge and full-sort selection. On exact candidate binary SHA-256 `02E2E136…EC877C`, 31 counterbalanced exact-output pairs improve hostile `allocation-survival` **111 → 110 ms, −2.93%** (descriptive bootstrap 95% CI **[−3.34%, −0.28%]**). A joint 21-pair `shapes-stable`/`shapes-megamorphic` check is neutral at **−0.07% [−0.81%, +0.40%]**; focused `polymorphic-objects-v2` is **−0.25%** with a wide neutral interval, and a 31-pair rerun of the initially noisy normal polymorphic row is **−0.06% [−0.44%, +0.54%]**. The complete normal all-13 suite is neutral across 15 pairs at **+0.09% [−0.62%, +0.56%]**, with exact outputs throughout. A focused Node diagnostic remains **1.786× Node** (108.2 vs 60.6 ms), so the per-row target remains open. Focused unit/integration tests, interpreter-only and safe-sandbox checks pass; the clean post-push `cargo test --workspace --release` gate exits 0 with the VM's **515 passed / 2 ignored** and every integration suite and doctest green. A separate MEM-to-nested-INT NanoID handoff was rejected and fully reverted after 32 balanced same-binary pairs made the row **+86.11% slower** [95% +84.96%, +88.03%]: the short child loop could not amortize an OSR host trip. |
| **B251 LANDED — scalar singleton `[[IsHTMLDDA]]` membership** | **hostile reactish-reconcile −4.11% [−5.05%, −3.75%]; normal all-13 0.9989× [0.990, 1.003]** | B251 LANDED (2026-08-30, `c575105`). The production VM creates exactly one `$262.IsHTMLDDA` exotic, but every `typeof`, loose-null comparison, truthiness check, callability check, and Annex-B call performed a randomized `HashSet<u32>` lookup. A scalar mirror now answers that singleton membership with one integer comparison; the set remains authoritative and `ZIPP_NO_HTMLDDA_SCALAR=1` restores it for same-binary ablation. `ZIPP_ICSTATS=1` proves the enabled and fallback routes are mutually exclusive. On the exact dirty candidate binary SHA-256 `468800E4…BD2D6F`, 31 counterbalanced exact-output pairs improve hostile `reactish-reconcile` **85 → 81 ms, −4.11%** (descriptive bootstrap 95% CI **[−5.05%, −3.75%]**). The complete normal suite is neutral across 15 pairs: all-13 **0.9989× [0.990, 1.003]**, headline-ten **0.9971× [0.992, 1.002]**, `ALL_CORRECT=1`. A focused 15-pair Node diagnostic is still **1.744× Node** (82 vs 47 ms, CI [1.734, 1.793]), so the every-row goal remains open. Focused release tests cover JIT/no-JIT, GC stress, kill switch, exact Annex-B semantics, and mechanism counts; interpreter-only and safe-sandbox checks pass. A separate plan-aware recycled-object-shell scan was rejected and fully reverted after making `shapes-megamorphic` **+5.87% slower** [95% +5.37%, +6.57%]. |
| **B250 VERIFIED — call-return string pins, direct Tier-C bases, emitted same-`FuncProto` guards, and typed-method snapshot repair** | **exact same-binary A/B: NanoID −12.63%; calls-closures −9.16% for the inline guard; all three paths hostile geomean 0.9693×; normal all-13 0.9971×** | B250 VERIFIED (2026-08-30). The MEM string-pin planner now admits Call-written receivers only when a forward MUST reaching-definition analysis proves that every path to each lookup has a current defining call: joins and backedges use AND, intervening definitions kill the fact, and handlers/finally fail closed. The post-call refresh keeps the raw string view licensed through converging two-call diamonds such as NanoID. Tier-C entry loads the fixed globals, heap-version, and IC-table bases directly from VM/JIT mirrors; the only growth points refresh those mirrors, and growth/re-entry tests cover later IC compilation and heap allocation. Same-`FuncProto`, capture-free leaf calls use an emitted heap-tag/bounds/fid/upvalue/realm-eval-global blocker guard before the unchanged helper fallback. Adversarial testing also found and repaired broader pre-existing license holes: call-free `SetProp` and pure `SetIndexConcat` writes now refresh pinned String/DataView/Array method snapshots; interpreter DataView dispatch and the JIT DataView helper both re-prove the live intrinsic before effects, including foreign-realm same-ID natives. PERFORMANCE on frozen final binary SHA-256 `AE7163D0…DF952`, all outputs exact: call-result pin on hostile NanoID, 31 alternating pairs, **138.097 → 121.088 ms, −12.63%** (95% CI −13.09% to −11.98%); inline same-proto guard on hostile calls-closures, 31 pairs, **55.928 → 50.848 ms, −9.16%** (CI −9.64% to −8.69%); the direct Tier-C bases independently measured **−8.39%** on that row before the final guard. All three switches off/on across every hostile case, 255 exact pairs, give **0.969315× (−3.07%)**, CI **[0.956984, 0.984268]**, with closures −15.16% and NanoID −13.47%; only types-stable (+0.82%) and bytecode-vm (+1.68%) regressed. The [normal same-binary A/B](../../bench/b250_combined_real13_abenv_2026-08-30.json) is regression-neutral at **0.9971×** (CI [0.993, 1.001]). Dirty non-PGO Node diagnostics are evidence, not publication captures: [normal](../../bench/b250_final_real13_node_2026-08-30.json) all-13 **0.7174×**, headline-ten **1.0269×**, 8/13 median wins; [hostile](../../bench/hostile/b250_final_dirty_2026-08-30.json) ordinary **0.9057×**, category-balanced **0.9460×**, 9/17 wins, calls-closures **1.198× Node** and NanoID **1.359× Node**. The every-row target therefore remains open, with eight hostile losses. VALIDATION: `cargo test --workspace --release` exited 0 (core VM 513 passed, two evidence tests ignored; every integration suite and doctest green), focused switch/mechanism tests cover eager/GC/no-JIT/helper-fallback modes, interpreter-only and safe-sandbox checks pass, and `git diff --check` is clean. |
| **B249 LANDED — the captured-call scaffolding closed: bare `MathOp`, fused computed calls, member-read arguments, no post-call register recycling** | **retained-ten 0.977× → 0.918× Node, all-13 0.665× → 0.635× (series best); hostile 0.970× → 0.866× (series best): bytecode-vm 3.68× → 0.93×, warm-router 2.18× → 1.62×; test262 95,929 → 95,936** | B249 LANDED (2026-08-30). THE CAPTURED-CALL SCAFFOLDING, closed. B248 restored the fused `CallMethod` where an argument could not observe the Get order and left three residuals; all three were the same thing seen from three sides -- the registers the hardening's captured lowerings leave in a loop body. (1) ORDER-TRANSPARENCY widened: a property / element / private read over transparent parts (`request.path`, `regs[a]`, `this.#p`) joins the primitive-operand class (user code only through an accessor or proxy trap on the ARGUMENT's own base -- the same exotic-partner assumption the class already makes for a global read), so `routes.get(request.path)` and `helpers[k](regs[a])` fuse again; `ZIPP_STRICT_CALL_ORDER=1` still narrows to the provable class. (2) The fused COMPUTED call (`obj[key](args)` -> `CallMethodComputed`, the op the INT computed splice and the MEM dense lane key on) is back for transparent arguments, in `e707e5d`'s exact shape. (3) A plain local callee that no argument can reassign is called from its own register (no snapshot `Move`: the pin planners key on a register's writer). (4) NO `next_reg` reset after a captured member call or a `MathOp`: recycling the callee/receiver temporaries let the next statement redefine them and the glob-range planner then saw one merged live range across the call -- the hostile bytecode-vm's spliced dispatch loop went 15 -> 16 homes, one over the pool, and its shared-home retry was refused by the GPR emitter with "split receiver has no used xmm home" (a decline that was SILENT; every INT-GPR decline now has a `ZIPP_JITLOG` reason, and `ZIPP_GLOBRANGE_DEBUG` lists the PERMANENT registers with their cause). (5) The computed splice gives each arm's captured `Math` pair DEDICATED window slots: mutually-exclusive arms share one scratch window, so arm 1's numeric temp landed on arm 0's captured receiver and made it a recycled pinned receiver. (6) THE BARE `MathOp`: `Math.<op>(args)` with order-transparent arguments captures NOTHING (`callee == NO_REG`, `this_v` = the `Math` global slot; eval/module re-index maps it like every other global operand, a by-name sentinel covers a slot the u16 cannot hold). The op validates the LIVE global slot and the LIVE `Math.<op>` own data slot at execution -- interpreter: `math_bare_is_intrinsic`, a per-op (heap generation, slot) memo whose slot VALUE is re-read every time because an in-place overwrite bumps no generation; JIT: the typed lane's `MathImulGuard` generalised per op (global slot bits, receiver generation, own-slot value, callable generation -- four compares, r12/r13, no registers), the INT tiers' region-entry `jit_math_imul_prefix_is_intrinsic` extended to bare globals; a miss performs the Get on the live global (`load_global_value`, the `LoadGlobal` arm's slow half factored out verbatim) and ordinary-calls the result. This is the pre-hardening register layout for every Math call with the hardening's semantics intact: `tests/bare_math_op.rs` pins replaced / restored / accessor / rebound-`Math` / deleted / shadowed against node in five child modes. Two Tier-C prologue predicates had to learn the form (a bare op reads r12 and r13 but carries no `LoadGlobal`/`GetProp` to pin them -- the first build segfaulted on warm-router), and the splice pair map had to skip it (its `this_v` is a global index that collided with a real arm register: the fixture's `Math.imul(x, 3)` read `[arg+1]` from the wrong window slot -- `computed_parity_virtual_retry_probe` caught it). PLAIN-BUILD SWEEPS (5 interleaved reps, outputs byte-identical on every row): hostile geomean 0.941x `e707e5d` where HEAD read 1.059x -- bytecode-vm 234 -> 46.5ms (node 46.8), warm-router 115 -> 85ms (e707 96), reactish 87 -> 85, shapes-stable 62 -> 61; real-13 0.906x `e707e5d` (HEAD 0.953x) -- markdown-render 293 -> 246ms, regex-log-scan 651 -> 501, json-large 351 -> 312, parse-large-js 268 -> 244, class-prototype-hot 224 -> 222. CONFORMANCE: the module trio (`<module source>` synthetic specifier was sent through `resolve_module_path` at link time) and the four cross-realm `Array.from`/`TypedArray.from` rows (three ShadowRealm-only "the main global object stands in for the realm" interceptions fired for a `$262.createRealm()` child, which has a global object of its own) are fixed -- both patches adversarially reviewed. CAPTURE (`0bff482`, PGO, 15 counterbalanced reps, ALL_CORRECT=1, publishable): retained-ten 0.9175× [0.914, 0.922] (from 0.977×), all-13 0.6347× [0.633, 0.637] (from 0.665×; series best), diagnostics 0.1858×; per row vs `271647a`: regex-log-scan 1.30× → 1.02×, json-large 1.21× → 1.04×, markdown-render 0.99× → 0.83×, parse-large-js 0.95× → 0.90×, class-prototype-hot 0.74× → 0.75×, typedarray-math 0.65× → 0.66×, async-promise-chain 1.26× → 1.24×, sparse-array 1.00× → 1.01×, map-set-heavy 0.93× → 0.93×, polymorphic-objects 0.94× → 0.93×; point wins 28/39 → 29/39, Bonferroni 26/39 → 29/39. Hostile cold ordinary geomean 0.9698× → 0.8662× [0.859, 0.871] (series best; 0.920× at `b2db432`): bytecode-vm 3.646× → 0.929×, warm-router 2.173× → 1.616×, throw-catch 0.471× → 0.421×, npm-nanoid 1.427× → 1.396×, types-churn 0.755× → 0.773×, async-lived 1.014× → 1.054×, the rest within ±0.02×. GATES: workspace release suite 1886/1886, safe-sandbox lib 457/457, standalone zipp-sandbox green, tier fuzz slice ok, test262 95,936/95,942 (only the six blessed rows). FOLLOW-UP (same day): the module loader now records `[[CycleRoot]]` at DFS time (InnerModuleEvaluation 11.c/16 on an explicit evaluation stack; `import defer`/`import source`/load-only requests take no edge), re-throws a recorded root error on every cache hit through a bounded lookup (the first version's mutual-reachability re-derivation was REJECTED in review for mis-attributing a deferred back-edge's error, an evaluation-order change and a per-cache-hit graph re-parse cliff: 200 re-imports after a failure 2846ms → 5ms), and stops GatherAsynchronousTransitiveDependencies at EVALUATING modules — `tests/module_cycle_root.rs` pins the reviewer's reductions; test262 95,939/95,942, the remaining three are the ES2017 annexB `arguments` assertion and the two `de`-locale `internalUsage` rows. OPEN, with the profile-first diagnoses this wave's five hostile scouts recorded (evidence, not charters): async-promise-chain 1.24× and json-large 1.04× on the retained ten; hostile allocation-survival 1.71× / reactish 1.68× / warm-router 1.62× / calls-closures 1.46× / shapes 1.43×,1.38× / nanoid 1.40×. nanoid: the outer MEM region pays a per-char `charCodeAt` helper on a Call-written string receiver (admit Call-written receivers as MEM STR pins with a post-call re-snapshot, est. −22..25%) and the compiled inner INT-GPR nest never runs because the outer MEM region owns it (nest composition); reactish: `diff`'s 134k self-calls take the FRAMED path because `for (const k of Object.keys(props))` leaves PushFinally/IterCloseFinally in the body (proto_has_handler_ops), five typeof/loose-null helper calls per invocation, the B212 frozen-head memo defeats the in-place concat chain (117k fresh strings), loop regions refuse every allocation op; survival/shapes: the recycle-pool address sort (run-aware sort), mirror-only death for pooled shells, a Tier-C dense-Array read lane, a plan-bucketed pool for the megamorphic 16-plan rotation (B239's Arc churn), post-major trim; calls-closures: Tier-C prologue pins as rdi-derived loads instead of three FFI accessor calls (−6.5%), the same-proto leaf guard emitted inline instead of `jit_leaf_same_func_proto` per inner call (−5..8%). Measured dead ends this wave (do not repeat): nursery budget ramps (all worse than adaptive), uniform props / nokeynull / elementinline on reactish (noise), delete/dictionary mode on mega (null), precise-pin/upval-forward/cross2/arrow-unbox latches on calls-closures (null). |
| **B248 LANDED — the 85fca98 regression root-caused and reversed: the JIT was speaking two call dialects** | **retained-ten 2.157× → 0.977× Node, all-13 1.227× → 0.665×; hostile 0.920 at `b2db432`; not captured at `9151043`× → 0.970×** | `85fca98` (190 files, no commit body) re-lowered EVERY `obj.m(args)` from the fused `CallMethod` to spec-order `GetProp; args; CallWithThis` (the property Get before the arguments — correct for an argument that can replace the property). Every method-call lane keyed on `CallMethod` went dead (the intrinsic push/charCodeAt arms, method inlining, the cross3 method lane, the DataView nested reduction, the Tier-C collection/random/toUpperCase lanes, leaf-inline admission), and compiled regions DEOPT-STORMED at the split `GetProp`: `jit_get_prop_miss` answered SELF_CALL_DEOPT for every prototype-method read on an Array/String receiver, so markdown-render's `closeLists` region deopted 16,983 times without ever evicting (a one-iteration loop decays its budget on every clean exit) and parse-large-js's regions evicted, blacklisted, and ran interpreted — 9.8×, 5.0×, 5.2×, 4.4× node on parse/markdown/regex/json. The published capture recorded it as "goal not met" because the harness said `publishable:true`; no A/B existed because compiler shapes and JIT lanes moved in one commit. Two more independent regressions rode along: every `Math.*` op gained a `LoadGlobal Math; GetProp f` prefix plus a per-execution helper doing a SipHash `HashMap<String,u32>` lookup (+42% on a Math-dense loop; and the prefix breaks leaf/nested inlining and the INT typed lane), and 16 new `next_reg` resets after calls recycled temporaries WITHIN an expression, giving pinned receivers two defs ("not cleanly excludable") and merging live ranges (typedarray-math 10 → 12 homes, INT-GPR declined, 156 → 571ms). THE FIX keeps the spec-order semantics where they are observable and restores the fused op where they are not: `arg_order_transparent` (compile/calls.rs) fuses a member call when every argument is a literal, `this`, a register-resident local, `!`/`typeof`/`void`/`&&`/`||`/`??`/`?:` of those (the provable class — no user code, no throw, nothing a getter could mutate), plus — unless `ZIPP_STRICT_CALL_ORDER=1` — reads of globals/cells/upvalues and arithmetic/concat/template/array/object/closure literals over transparent operands (user code only through an object operand's coercion, the same assumption every call site made before the hardening). Member and index reads, calls, spreads and assignments stay captured; regexp spellings (`test`/`exec`/`matchAll`/`replace`) stay captured for the `RegExpMethod` direct lanes (fusing them cost the whole matchAll phase, 58 → 309ms). The split form is now first-class where it remains: `jit_get_prop_miss` serves the prototype DATA slot for Array/String/Cons/Map/Set/WeakMap/WeakSet/Date/Promise receivers (fail-closed on side-table shadows, custom prototypes, accessors, proxies, index keys, foreign realms; memo-first through B191's version+slot+bits proof for Array/String — the chain walk alone was parse-large-js's `src.charCodeAt(starts[ti])` phase, 94 → 300ms), a bits-guarded captured `push`/`charCodeAt` lane runs the dedicated helpers from the MEM region and Tier-C emitters, native callees take a stack-buffered argv in both `CallWithThis` paths, `slot_guard_def` models `CallWithThis` (the INT splice had declined "unmodelled-op-in-scan"), the captured callee is allocated above the function's high-water mark so its `GetProp` def can never share a pinned receiver's register, the receiver snapshot `Move` is skipped for a register-resident local no argument assigns (bytecode-vm's pin), the fused path never resets `next_reg`, and the 16 added resets are gone. `Math.*`: `MathIntrinsicGuard` bakes every intrinsic's bits+generation per op (three inline compares, the `imul` path generalised) and `builtin_ns_slots` replaces the string-keyed map in `static_builtin_is_intrinsic` (mathfloor micro 102 → 59ms = node). Method: plain `cargo build --release` of HEAD was SLOWER than the PGO binary (so not a PGO artifact); `zipp bc` bytecode diffs against the kept `e707e5d` binary and `ZIPP_JITLOG` deopt counts per row were the categorical signals; per-phase timers inside regex-log-scan and parse-large-js localised each residual; a three-investigator workflow independently root-caused the deopt storm (patch adopted), the dead switches and the Math guard, with adversarial verification. Tests re-pinned: zero/pure-argument calls fuse (`tierc_random_method`, `cold_pinned_recv`, `w20_int_push`, `instr_uses_exhaustive` gain a `CallMethod` case and impure-argument siblings for the captured order). CAPTURE at `271647a` ([`real13_271647a_pgo_2026-08-29`](../../bench/real13_271647a_pgo_2026-08-29.json), [`head_clean_271647a_pgo_2026-08-29`](../../bench/hostile/head_clean_271647a_pgo_2026-08-29.json), 15 reps, exact, `publishable:true`): retained-ten 0.9773× [0.971, 0.983], diagnostic 0.1838×, all-13 0.6646× [0.661, 0.668] (series best; 0.682× at `b2db432`), 28/39 median wins, startup 7.2ms; hostile overall 0.9698× [0.965, 0.974] with mixed locals crossing under Node (1.17× → 0.76×), stable/megamorphic shapes, survival, reactish and NanoID all ahead of `b2db432`. RESIDUALS (diagnosed here; all three closed by B249): hostile bytecode-vm 3.68× (0.92× at `b2db432`: its INT-GPR dispatch region is one home over the register pool once the computed call is captured), warm-router 2.18× (1.69×), throw-catch ≈+10%; regex-log-scan 1.30× and async-promise-chain 1.26× read behind their `b2db432` figures (1.21×/1.09×) while the plain builds of `271647a` and `e707e5d` measure at parity on both — attributed to the B246 training boundary (the profile no longer sees `bench/real`), an inference for the PGO binaries; nested wrapper inlining (`ri()`→`rnd()`) still declines because `shift_leaf_regs` cannot renumber a `GetProp` (the `Math.*` prefix in the inner leaf needs a name remap). CONFORMANCE: the full test262 sweep at `271647a` read 95,909/95,942 -- 24 rows below the historical 95,933, ALL of them present at `768a2a8` and absent at `e707e5d` (the mega-commit, not B248): twenty `built-ins/RegExp` + `staging/sm/RegExp` constructor rows (`RegExp(re) === re`: direct `RegExp(x)` calls stopped being compile-lowered to `NewRegExp{is_construct:false}`, the op that carries step 22.2.4.1-3, and the plain-call path never had it -- restored in `9074b36` at `call_ctor_as_function`, 95,929/95,942), and four cross-realm `Array.from`/`TypedArray.from` rows (`staging/sm/*/from_realms.js`; the `array_from` body is byte-identical to `e707e5d`'s, the difference is in `85fca98`'s `native_callee_realm` plumbing) -- recorded, not blessed. |
| **B247 CAPTURE — the `9151043` PGO capture, plus bounded whole-repository verification on Windows** | **publishable and exact — and, per B248, a REGRESSION reading: retained-ten 2.157× was `85fca98`'s de-fused call lowering, not the engine's standing** | [`real13_9151043_pgo_2026-08-29`](../../bench/real13_9151043_pgo_2026-08-29.json), 15 counterbalanced repetitions, Node 24.12.0 / Bun 1.3.14 / Deno 2.6.10, `publishable:true`, `ALL_CORRECT=1`: retained-ten Zipp/Node **2.1567× [2.150, 2.179]**, diagnostic-three **0.1875× [0.185, 0.190]**, all-13 **1.2274× [1.223, 1.238]**. All-13 is **1.0698× Bun** and **1.0583× Deno**. Zipp wins 20/39 paired medians and 18/39 Bonferroni exact-sign comparisons, so the strict target remains false; startup is **7.7ms** versus Node 31.7ms. The first attempted capture also found the new full-repository byte verifier exceeding Windows' command-line limit at 944 tracked paths. `git ls-tree` pathspecs are now count/length bounded in batches, covered by the 163-test tool suite; the live verifier returns clean on all 944 paths. PGO binary SHA-256 `91ae5e38…c7962e0b`, profile `34ab30dd…746ed273`, source `9151043`. |
...
| **B246 SECURITY/MEASUREMENT HARDENING — bounded regex transients, canonical benchmark children, and corpus/build-bound PGO provenance** | **no speed claim; publication now tests the literal all-engine goal** | Safe regex matching charges backtracking, scratch, captures, retained replacement lists, and recursively nested functional replacers against one VM-wide scoped allowance. The standalone sandbox remains no-JIT/secure-allocator. Both benchmark harnesses now launch all four native engines from a from-scratch isolated environment, stage selected inputs before probes, require a full clean-repository/HEAD-before-after envelope, and compute per-row Zipp ratios against Node, Bun, and Deno. “Faster on every row” requires every exact one-sided paired sign test to pass its Bonferroni-adjusted threshold at **95% family-wise confidence**; bootstrap intervals remain descriptive. PGO training excludes every tracked non-training JavaScript/module benchmark plus all manifest-declared hostile inputs, and binds their paths and byte digests as publication data; this is an exact-file anti-leakage boundary, not a claim of statistical independence. Both Cargo stages compile one private read-only clean-commit snapshot. Binary identity binds the release envelope and the selected Cargo/rustc/rust-lld and MSVC driver bytes; SDK headers, import libraries, and compiler backend DLLs remain developer-environment-path scoped rather than byte-manifested. |
...
| **B228+B229 LANDED — and the earlier “B228 refuted” verdict is ITSELF RETRACTED: the admission was refuted by MY OWN spurious generation bump** | **types-churn −13.09% (67.8→58.9ms, 21 reps; 1.338× → ~1.16×) — the row that three previous waves left untouched; 4-row geomean −3.41% [−4.14,−2.79]; retained-13 −0.44%, no eviction churn** | B227's follow-up was to admit a callee with >64 registers when its WIDE fill mask is entirely empty (types-churn's callee has 94 registers and a wide mask reading `marked=0` — nothing to fill — yet cross3 declined it purely because its plan carries a `u64`). Building it surfaced a REAL LATENT BUG first: `set_cross_entry` bumps `mask_gen` only when the INLINE mask changes, and a wide callee's inline mask is permanently `u64::MAX`, so a recompile that changed which wide registers may be read-before-written would never bump the generation — every dependent lane's baked fill silently stale. Harmless only because nothing baked a wide mask yet; a live wrong-answer the moment anything did. That fix LANDS (the wide mask now participates in the comparison), inert today and a footgun removed. The admission itself does NOT: with it, types-churn's site does emit the lane (`CROSS3 fn1 native-emitted lane` where the gate previously never logged) and answers correctly — but its helper fill count is UNCHANGED at 1,199,983, so those calls enter the helper from somewhere the lane does not cover, and the A/B is null. The first attempt measured +0.29% (null) and I recorded it as refuted — WRONGLY. Keeping the diagnostic rather than accepting that null is what found the truth: a `mask_gen` bump log showed the callee's generation bumping with `wide_changed=true` and the INLINE mask unchanged, because a first `None -> Some` set counted as a change. So the callee bumped its generation immediately AFTER the caller baked the lane, and the lane I had just enabled guarded false from its first call. TWO further fixes were needed, and together they are what makes the admission real. (1) `wide_changed` now requires a PREVIOUS LIVE ENTRY — with no prior entry nothing could have baked anything, so a first set is not a change. (2) B229, the missing half of B199: `cross_pending` re-plans a caller that planned while an entry was MISSING, but NOTHING re-planned a caller whose baked lane was later INVALIDATED. types-churn's exact sequence, visible in the compile log: `fold` compiles → `main@8` bakes its lane → `main`'s hot region compiles → **`fold` is EVICTED** by the direct-miss site gate → `fold` recompiles with a different shape. From the eviction onward the caller's lane guarded false for the rest of the run. A new `cross_baked` map (callee fid → callers that baked against it, mirroring `cross_pending`) now re-plans those callers on a generation bump, under the same B199 discipline: park the artifacts, let hotness re-trigger, no blacklists. LESSON, recorded twice over now: a null is a claim about the CODE AS BUILT, not about the idea — when a mechanism should work and measures null, instrument the mechanism before believing the number. |
| **B231 CHARTER (designed to the instruction level, deliberately NOT built at the tail of a long session): capture-aware leaf inlining for SAME-PROTO sites** | est. the largest remaining bounded lever — calls-closures is 98.0% jit-mem with zero GC, and its INT-tier decline is `Call`, so flattening the callee is the only route onto the register tier (the B223 pattern that took parse-large-js −22.5%) | Capturing closures are ALREADY inlinable — `build_leaf_inline_plan` resolves each cell from the exact closure its identity guard pins, and the code's own comment records that gate having been worth 6.3× on the shape it unblocked. What is refused is the SAME-PROTO case: `if same_proto && (callee.lexical_this || !callee.upvalues.is_empty()) { continue }`, because a same-proto site sees MANY closure instances sharing one FuncProto, so no single instance's cells can be baked (`LeafInlinePlan::upvals` is documented as exactly that bake). calls-closures hits this nine times — its callees are rotating closures over mutable cells, and `ic_call_mono` returns None for them, so the capture-capable mono path never applies. THE FIX IS TO READ THE CELLS LIVE, and every piece already exists: `Heap::upvals_mirror_raw` is maintained (written at closure birth, cleared in `free_slot`, and the pointer is fixed for the closure's lifetime because the box never moves), and `upvals_mirror_of` already serves the helper side. It has NO emitted offset only because nothing needed one — `host_api.rs`'s own comment says so. So: add `JIT_UPVALS_MIRROR_RAW_OFFSET` beside `JIT_THIS_MIRROR_RAW_OFFSET` (three lines, same `offset_of!` pattern); give the plan a dynamic-upvalue mode instead of the baked map; and in `inline.rs`'s `UpvalGet` arm replace `mov rdx, QWORD cell_bits` with a load of the live callee's index → `[upvals_mirror_raw + idx*8]` → `[base + i*8]` → the same `cell_get` call. Four instructions where there was one immediate. The upvalue COUNT needs no new guard: it is a property of the FuncProto the same-proto guard already pins. **BUILT, MEASURED NULL, REVERTED — and the reason is the useful part: calls-closures needs THREE gates lifted, not one.** The design above works and was implemented in full: `JIT_UPVALS_MIRROR_RAW_OFFSET` added, a capture-tolerant twin of `jit_leaf_same_func_proto`, `dyn_upvals`/`dyn_this` plan modes, and an emitter that loads the cell index from `[upvals_mirror + idx*8][i*4]` and rebuilds the tag (plus the same trick for an arrow's `this` via the EXISTING `JIT_THIS_MIRROR_RAW_OFFSET`, which cross3 already uses). Outputs verified byte-exact on all seven hostile rows. It measured **+0.35% geomean across five rows — null** — and the JITLOG says exactly why: lifting the capture gate moved calls-closures' decline from “same-proto leaf has captures or lexical this” to “**same-proto target is not a direct leaf**”. Its hot callee is `(value, salt) => {…}` which WRAPS an inner `rotate(…)` call, and same-proto inlining stops at direct leaves by design. The nested (`callee_leaf_ok_one_call`) path exists but BAKES the inner closure's cells from one pinned instance — the identical problem one level down — so admitting same-proto there needs live inner cells AND a relaxed inner identity guard. **CORRECTION after reading the bench rather than assuming:** part (3) is NOT live inner cells. `rotate(value, amount = 5)` closes over NOTHING — it reads only its own parameters — so `inner_upvals` is empty and there is nothing to bake wrongly. What the inner call actually needs is a SAME-PROTO identity guard of its own: `makePipeline` is invoked 16 times, so 16 distinct `rotate` function objects share one FuncProto, and the nested splice today bakes one identity. The outer arrow captures four bindings (`calls`, `offset`, `seed`, `rotate`) and is an arrow, so its cells and `this` must be read live — which part (1) already does, and which is also what supplies the inner callee value, since `rotate` reaches the arrow through one of those cells. So the real shape is TWO parts, not three: (1) live outer cells + live arrow `this` [built, proven, byte-exact on seven rows], and (2) same-proto admitted to the nested path WITH a same-proto guard on the inner callee. Land them together and measure the combination; (1) alone measures null, which is why it is not in the tree. Note for expectations: even a full success leaves calls-closures around 1.2–1.3×, so this is a gap-narrowing move, not a parity crossing. |
| **B232 REFUTED (built, measured by telemetry, reverted) — but it QUANTIFIES the one lever left on json-large, which is the row the hardening taxes hardest** | the pooled-key change serves 4,096 of 444,918 keys (0.9%); what the attempt measured instead is that **JSON.parse performs ~440,000 key allocations per run** | The hardening tax is proportional to ALLOCATION COUNT, so cutting allocations on json-large is engineering rather than the policy question its 1.27× usually reduces to — worth an attempt. Its `[keypool]` telemetry looked like free money: the pool sat FULL (recycled=4096, resident=4096) and served ZERO, because JSON.parse was the only heavy key producer never asking for a buffer. Wired it in (`as_str_lossy` borrows for a well-formed key, so the pooled buffer is filled with nothing allocated first) — outputs byte-identical, and the serve rate went to **4,096 of 444,918**. The pool cannot help here by construction: a parsed object's keys are LIVE, so they never return to the pool during the parse, and only four collections run. REVERTED. **What the run does establish is the target:** json-large is 31.0% json-parse + 24.6% json-stringify + 7.0% GC, and ~440k of its allocations are object KEYS whose text repeats across every node (the same handful of field names, 150,021 nodes deep). Every one is a separate `String` because `ObjMap` owns its keys by value. Interning them — the key-representation refactor to a shared `Rc<str>`/id, the same insight B212 exploited for `“prefix” + int` — is the lever, and it would cut the row's allocation count AND the hardening tax that scales with it. Large, but now quantified and no longer a policy dead-end. |
| **B233 LANDED: `JSON.parse` reads an escape-free member name STRAIGHT OUT OF THE SOURCE — and the sequence-plan idea it started as is REFUTED by the bench's own key generator** | **json-large −4.00% (363.0 → 348.4ms, 21 alternating latch pairs, ON faster in 18/21; min-vs-min −1.25%)** | B232 left json-large quantified: ~440,000 object-key allocations per run, 31.0% of the row in json-parse. The first attempt took the bounded route to B232's stated lever — reuse `PropKeys::Planned`, cache a `StaticKeyPlan` per repeated key SEQUENCE, and let repeat nodes reach `finalized_from_plan` (shared keys, memoized shape fold, `AllData` attrs). It measured **+0.09%, and instrumentation says why: hits=0, misses=111,624.** Reading the bench instead of assuming: json-large's keys are `WORDS[ri(256)] + "_" + j`, so a node's key SEQUENCE is drawn from 256^k possibilities and repeats essentially never — the WORDS repeat, the sequences do not. Sequence-level plans cannot serve this corpus by construction. **REFUTED, and the distinction is the useful part: B232's lever is per-STRING interning, not per-shape planning, and the two are not substitutes.** What survived is the primitive built to support it. `json_parse_string` reaches a key's `String` through a `Vec<u8>` and a `JsStr` and then `to_lossy_string` — three allocations per key where one will do — so `json_scan_plain_key` returns the source bytes borrowed when the name holds no escape, no control character and is valid UTF-8, and the caller allocates the one exactly-sized `String` the map will own. Anything else consumes nothing and re-reads through the general parser, which keeps every escape, lone-surrogate and error rule in one place. Latch `ZIPP_NO_JSON_PLAIN_KEY`. Pinned by `tests/json_plain_key.rs` — 21 node-oracled cases run against BOTH latch states, covering each escape kind that must refuse the shortcut, duplicate replacement, `__proto__` as an own property, canonical-index enumeration order, descriptors, post-build mutation, `for-in`, spread, freeze, a reviver, and a hot polymorphic read site. |
| **B233 side-finding (CORRECTNESS, PRE-EXISTING, not introduced and not fixed): a JSON member name holding a LONE SURROGATE is folded to U+FFFD** | no speed claim — recorded so it is not re-discovered as a regression | `JSON.parse('{"\\ud800":1}')` gives node a key of length 1 with code unit 55296; zipp gives 65533. Confirmed pre-existing by running the same case with `ZIPP_NO_JSON_PLAIN_KEY=1`, which restores the exact prior code path and prints the same 65533. The cause is representational rather than a bug in the parser: `ObjMap` keys are Rust `String`s, so a lone surrogate has nowhere to live and `to_lossy_string` is doing the only thing it can. Values are unaffected — only property NAMES. Closing it means the same key-representation change B232 charters for allocation reasons (a WTF-8-capable shared key type), which is why the two belong in one piece of work rather than two. Deliberately NOT pinned by `tests/json_plain_key.rs`, whose header records the exclusion. |
| **B234 LANDED: `JSON.stringify` stops DECODING each string a second time just to size it** | **json-large -2.29% and -2.37% in two independent 21-pair latch replications with opposite lead order (min-vs-min -2.17%); output byte-identical (`jsonLen=1835828`, `jsonHashAcc=601658468`). Stacked on B233 the row is 363.0 -> ~342ms, -5.8%** | Every stringified string is measured before it is written, because the output buffer is grown FALLIBLY against `MAX_STRING_BYTES` and the reservation has to be exact — `json_quote_into`'s own `reserve` is infallible, so an under-reservation would hand `String::push` an aborting allocation path. The exact size came from a full second decode (`wtf8_code_points` for heap strings, `chars()` for `&str`), and heap strings are what every key and value of a stringify actually is. `json_quote_len_is_plain` settles it with a byte scan instead: for PRINTABLE ASCII with no `\"` and no backslash, each byte is a whole code point, is not escaped, and cannot be part of a multi-byte or lone-surrogate sequence, so the quoted length is exactly `len + 2`. Deliberately NOT extended to multi-byte UTF-8: `wtf8_decode` answers malformed input with U+FFFD, whose encoded length differs from the bytes it consumed, so a byte scan could disagree with the writer on input this predicate cannot rule out — and the reservation must not merely be close. Everything else keeps the exact loop, so the reservation stays exact and the fallible-growth discipline is untouched. Latch `ZIPP_NO_JSON_QUOTE_FASTLEN`. |
| **B244 REFUTED (built, measured, reverted): `is_empty()` guards on the two side-table probes inside `jit_ta_snapshot`'s array arm** | **shapes-stable +0.50% (8/15), megamorphic +1.63% (5/15), calls-closures +0.91% (7/15), warm-router +0.81% (4/15) — null, two-binary against the kept `e707e5d` build** | The HEAD profile has `jit_ta_snapshot` at 3.6—6.2% on the three call-heavy rows that keep an array pinned in the loop, and the lane's own comment says why it runs so often: a pinned array's `{base, len}` is re-derived after EVERY cross call inside the region, because the callee may have grown it. The arm's two `HashMap::contains_key` probes (`arr_props`, `arguments_objs`) looked like the B242 pattern — maps that are empty in every ordinary program, asked per call. Guarding both with `is_empty()` measured null on all four rows, so the probes on an empty table cost nothing worth having (FxHash of a `u32` plus one control-group load) and the helper's 3.6—6.2% is the call itself: prologue, the closure, `heap.get`, three stores, epilogue, once per cross call. The same conclusion B242 reached for the leaf guard, and the same consequence: **only an emitted re-derive removes it**, which needs the emitter to know where an Array occupant's storage pointer and length live inside `HeapObj` — the same layout knowledge an inline `regs.len` (B243) and an inline flat-string `.length`/`charCodeAt` on the MEM tier (nanoid's 22%) need. One primitive, three levers; that is the design question now, not another probe guard. |
| **B242 LANDED (profile-guided): the allocation poll goes inline and the collector goes cold; two empty-map probes leave the same-proto leaf guard** | **types-churn —3.28% (18/21), shapes-megamorphic —1.49% (17/21), reactish —1.22% (14/21), allocation-survival —1.12% (14/21), warm-router —0.89% (13/21); calls-closures NULL (+0.66%, 5/15). Two-binary against a kept copy of the `d7836fc` build with an A/A control (rebuilt `d7836fc` in a worktree) within ±0.5% on every row above 30ms; outputs identical on all seven rows** | The HEAD profile (`d7836fc`, 25 runs per row) put `maybe_gc` at 2.2—2.6% on five rows. It already carried `#[inline]`, yet the linker map of the profiling build has NO `Vm::gc` or `sweep_young` symbol — the collector had been inlined INTO the poll, so the poll could not itself inline into the JIT allocation helpers, and every object paid a call to reach four loads. (The first cut added a second `#[inline]`, which the compiler flagged as a duplicate — the map, not the attribute, was the answer.) `maybe_gc` now calls a `#[cold] #[inline(never)] gc_from_poll()` shim: the collector exists once, out of line, and the poll is the loads alone. That is the whole of the measured win, consistent across the five allocation-heavy rows. The other half was a miss, recorded as such: `get_function_realm` (4.5% of calls-closures in the profile) probes the `obj_realm` map per call and `jit_leaf_same_func_proto` (8.5%) probes `closure_eval_scope` per call, both maps empty in every ordinary program; `jit_calls.rs` already guarded its own `get_function_realm` call with `!obj_realm.is_empty()`, the leaf guard did not. Both now early-out on an empty map — correct and cheaper, and **null on calls-closures**, so those two symbols' 13% is not the hash probes; the helper's self time is its own body plus the call, which only an emitted guard removes. **The HEAD profile, so the next session does not re-derive it.** Shared across the six allocation rows after B238/B239: `free_slot` 5.9—12.3%, `alloc_finalized` (refit inlined) 3.7—15.2% (megamorphic highest: alternating plans defeat `ptr_eq`), `alloc_settled` 3.2—6.3%, `jit_cross3_enter` **4.6—7.3% on six rows**, `mi_page_free_list_extend_secure` 2—3% (the hardening). calls-closures: 69% emitted, then `jit_leaf_same_func_proto` 8.5, `jit_cross3_enter` 7.3, `jit_ta_snapshot` 6.2, `get_function_realm` 4.5 — pure call protocol. npm-nanoid: `jit_quick_len` **15.3%**, `str_append_index_ascii_fast` 7.0, `proto_method_is_baseline` 6.9, `jit_char_code_at` 6.5 — and the JIT log explains it: the bench's hashing loop `[45,58]` is compiled as an INT-GPR region with `id` pinned (inline `movzx` charCodeAt, hoisted length) but the OUTER loop's MEM region `[27,61]` swallows it and runs it with three helper calls per character; int-splice cannot rescue it because a splice wraps a LEAF CALL and this loop has none ('no leaf plan'). A census over all 30 benches finds that nested shape on nanoid ALONE, so a MEM—INT nested-region entry is a one-row lever (~—24% of nanoid, not a crossing). `jit_ta_snapshot` 3.6—6.2% on three rows is the per-region-entry pin snapshot, whose STR arm re-proves `String.prototype.charCodeAt` on every entry. async-lived is interpreter-bound (`dispatch_body` 17.9%) and at parity. |
| **CAPTURE #14 (`b2db432`, clean PGO, 2026-08-28): hostile 0.9356 — 0.9200 (series best); retained-ten 0.971 — 0.9648 (series best); all-13 0.682** | **Hostile per row (ratio, previous — now):** shapes-stable 1.657 — 1.525, reactish 1.862 — 1.777, bytecode-vm 0.960 — 0.918, types-churn 1.197 — 1.169, megamorphic 1.604 — 1.570, warm-router 1.703 — 1.686, npm-nanoid 1.475 — 1.455, calls-closures 1.483 — 1.473, survival 1.901 — 1.897, async-lived 1.007 — 1.000 (at parity within noise); rows above parity 9 — 9. Node ran ~2% slower than at the previous capture on every row, so the ratios are the comparison. **Real corpus:** json-large 1.27 — 1.19 (B233+B234), map-set-heavy 1.00 — 0.97, markdown 0.95 — 0.94, async-promise-chain 1.10 — 1.09, regex-log-scan 1.19 — 1.21 and polymorphic-objects 0.99 — 1.01 (both within noise, no covering change). Startup 9.3ms (node 31.0). | Artifacts: [`head_clean_b2db432_pgo`](../../bench/hostile/head_clean_b2db432_pgo_2026-08-28.json), [`real13_b2db432_pgo`](../../bench/real13_b2db432_pgo_2026-08-28.json). README refresh #14. The two `476c660` captures that preceded it (0.9562 and 0.9605) are the ones that caught B240's startup regression and measured the capture-to-capture noise floor; they are kept as evidence, not as headline. **Where the goal stands after this capture:** nine hostile rows above parity (survival 1.90, reactish 1.78, warm-router 1.69, megamorphic 1.57, shapes-stable 1.53, calls-closures 1.47, nanoid 1.46, types-churn 1.17, async-lived 1.00) and three real rows (regex-log-scan 1.21, json-large 1.19, async-promise-chain 1.09) with sparse-array and polymorphic-objects at 1.01. The profiler now names the next levers instead of the charters guessing at them. |
| **B241 LANDED (review polish for B237—B240): the fifteen skeptics' lesser findings, applied** | no speed claim; startup B234 6.79 — HEAD 6.69ms (−1.6%, 17/21) confirms no import-table or code-size regression from a commit that touched `extern` blocks; hostile outputs byte-identical to the B234 binary; binary 7,685,632 — 7,691,264 | Everything the B240 review found short of a refutation, in one commit. **Profiler.** `report()` now actually stops the sampler (`THREAD.swap(0)` then a wait on an `INFLIGHT` counter the armed path raises BEFORE re-reading `THREAD`, both SeqCst — Dekker-style, so a sample either backs out or is waited for), which makes the 'after sampling has stopped' docs true and removes the one-stale-slot artefact. Sample slots are `AtomicU64` with Relaxed stores/loads (plain movs on x86-64), so the shared buffer is well-defined rather than a formal data race. The disarmed path is a single relaxed load as the doc always claimed. `register()` takes the label as a closure, so the nine JIT install sites build no `String` in an ordinary run, and it evicts every existing entry the new range overlaps — the RANGES doc's premise was wrong (an evicted body is PARKED in `retired`/`retired_fns`, not unmapped; only `Jit::set_meter` and a whole-VM drop unmap, and `$262.agent` worker VMs register from their own threads), and after an unmap a new mapping can reuse the range, which stale entries would have mis-resolved. The `SYMBOL_INFO` buffer is `#[repr(C, align(8))]` (a bare `[u8; N]` is align-1 and the `u32` header writes were formally misaligned). dbghelp loads via `LoadLibraryExA(.., LOAD_LIBRARY_SEARCH_SYSTEM32)` so the application directory is never searched. `DROPPED` is reported. **Heap.** `refit_finalized` carries the two `debug_assert`s `finalized_from_store` has (`runtime_valid`, `vals.len() == plan.len()`), so the two spellings are identical in debug builds too; `alloc_object_settled` asserts its shape contract (`boxed.shape() == shape`) instead of trusting its single caller; the three comments still describing the pre-B239 clone-then-drop of the shell's plan Arc now describe the refit. **Correctness hardening (pre-existing, unreachable from JS).** Both `FinalizeObject` arms — the baked JIT helper and the interpreter — staged values into a 16-slot stack buffer behind a `> 256` guard. The compiler caps a finalize literal at `OBJECT_FINALIZE_MAX_FIELDS = 16`, so no compiled program can reach it, but a hand-built `Program` handed to the pub `Vm::new` with a 17-key plan indexed past the buffer (abort in the JIT helper, panic in the interpreter). Both checks and both buffers now use one `FINALIZE_STAGE_SLOTS = 16` constant, homed in `bytecode.rs` because `object_literal_jit` is `cfg(jit)` and the sandbox build could not see it there (the first cut failed exactly that gate), and the compiler's `OBJECT_FINALIZE_MAX_FIELDS` is now DEFINED FROM it, so the admission cap and the runtime buffers cannot drift apart. **Deliberately NOT changed:** the interpreter `FinalizeObject` arm and the general (non-baked) JIT helper still `.cloned()` the plan before `alloc_finalized(&plan)`, so B239's 'zero atomics' holds on the BAKED path only — the one every hot literal on the hostile rows takes; the other two are cold-tier and the borrow restructuring to pass a table reference through `&mut self` is not worth its own risk here. |
| **B240 LANDED (REGRESSION FIX + method rule): the capture caught a fixed +1.5ms process-startup cost that NO LATCH could see — B237's `#[link(name = "dbghelp")]` put dbghelp.dll in the import table, and B238's forced `inline(always)` added 664KB of code** | **startup 6.73ms (B234) → 7.84 (B237) → 7.88 (B238) → 8.12 (B239) → 6.56 after B240 (B234 6.46, +1.57%, 7/21 — null). Against the pre-profiler B234 binary, HEAD now reads types-stable +0.04%, ephemeral +0.09%, calls-baseline +1.01% (9/21), and keeps shapes-stable −8.19% (21/21), megamorphic −4.51% (20/21), survival −2.33%, warm-router −1.49%, types-churn −1.32%, reactish −1.15%, calls-closures −1.06% (20/21). Binary 8,337,408 → 7,685,632 bytes** | The clean PGO capture at `476c660` ([`head_clean_476c660_pgo`](../../bench/hostile/head_clean_476c660_pgo_2026-08-28.json)) read geomean 0.9356 → 0.9562 — shapes-stable −8.16% as the latches predicted, but types-stable +9.25%, ephemeral +9.31%, calls-baseline +8.93%, module-hot-graph +7.28%. Per the standing rule a capture-to-capture move without a covering A/B triggers a latch bisect, and the bisect on the PGO binary itself said B238+B239 were NULL on every short row (win rates 8–14/21, a coin flip) while still reading −9.65% (21/21) on shapes-stable, −4.04% survival, −3.39% warm-router. So the regression was real and NOT in anything latched. The absolute times said what it was: node moved +0.5–3% across the corpus, zipp moved **a flat +1.5–1.7ms on every short row** (ephemeral 14.2→15.7, calls-baseline 16.8→18.3, module-graph 16.8→18.4, types-stable 17.4→19.1) — a fixed per-process cost, invisible on a 100ms row and +8–10% on a 15ms one. A one-line script measured it directly, 6.58 → 8.03ms (0/21), and a per-commit bisect put **+1.1ms at B237** and a further +0.3ms across B238/B239. Two causes. (1) B237 declared dbghelp's three entry points in a `#[link(name = "dbghelp")]` block, which puts the DLL in the IMPORT TABLE, so the loader maps it before `main` in every run, profiling or not. Fixed by resolving the same three functions with `LoadLibraryA`/`GetProcAddress` at REPORT time; the profiler is the only caller and runs once after sampling. (2) B238's `alloc_slot` split needed `#[inline(always)]` to stop the generic path becoming a call, and the forced inline grew the binary from 7,673,344 to 8,337,408 bytes. Fixed by passing the known mirror as `Option<HotMirror>` through ONE body (`alloc_settled`); `alloc` is a wrapper LLVM folds away, and the settled path passes `Some`. Verified from the linker map of the `b2db432` profiling build: exactly one out-of-line `Heap::alloc_settled`, and `Heap::alloc`, `alloc_object_settled`, `alloc_finalized` and `refit_finalized` have no symbol at all (folded into their callers), which is what 'a wrapper LLVM folds away' has to mean. Every B238/B239 win survives the single-body form (shapes-stable −8.19% vs the −8.1% the pair predicted). **Two rules recorded.** A latch can only disable a fast path; it cannot see a cost a change adds UNCONDITIONALLY — an import-table entry, code-size growth, a new static initializer — so any wave touching linking, `extern` blocks, inlining attributes or build profiles must also run the two-binary startup check on a one-line script against the pre-wave build. And the short rows are the detector for it: a fixed cost shows up there first and largest, which is a reason to keep them in the corpus rather than a reason to discount them. **Capture-to-capture noise floor, measured:** a second capture of the IDENTICAL binary ([`repeat_476c660_pgo`](../../bench/hostile/repeat_476c660_pgo_2026-08-28.json)) read geomean 0.9605 vs 0.9562 and moved shapes-stable +6.1%, npm-nanoid +2.5%, reactish −1.7% with nothing changed. Per-row moves under ~5% between captures are not evidence of anything; the latch A/B is the evidence, and the capture is the smoke detector. **Adversarially reviewed** before landing: 15 independent skeptics over five claims (refit equivalence for every pool-admissible shell state, settled-mirror bit-identity on both alloc branches, plan-by-reference soundness and the zero-atomics trace, sampler safety, code-range registration). The four correctness claims survived every vote; the three dissents all hit the profiler's cost-accounting WORDING (`sample()` does two relaxed loads, not one; install sites build the label `String` before `register()` bails; `$262.agent` worker threads also register) and are fixed in the follow-up polish commit together with the review's lesser findings: the two `debug_assert`s `refit_finalized` dropped, a `debug_assert_eq!` on `alloc_object_settled`'s shape contract, an align-1 `SYMBOL_INFO` buffer (formal UB), three stale comments still describing the pre-B239 clone-then-drop, the RANGES doc's wrong premise (evicted bodies are PARKED in `retired`, not unmapped — only `set_meter` unmaps), a sampler that never actually stopped, and a pre-existing 16-slot staging buffer guarded by a 256 check on both FinalizeObject arms (unreachable from JS, reachable from a hand-built Program). |
| **B239 LANDED (profile-guided, second wave): a pooled shell is REFITTED in place, and the key plan travels by reference** | **shapes-stable -6.21% (21/21 pairs), allocation-survival -3.65%, shapes-megamorphic -3.07%, types-churn -1.60%, warm-router -1.56%, reactish-reconcile -1.41%. Outputs byte-identical on all six plus json-large, polymorphic-objects and class-prototype-hot with the latch both ways. With B238 the object-lifecycle pair is worth ~-8.1% on shapes-stable, ~-4.5% megamorphic, ~-4.1% warm-router, ~-3.9% survival and ~-3.1% reactish** | B237 priced `drop_in_place<ObjMap>` at 4.3-7.5% on the four biggest hostile rows, and the pool telemetry says where those drops come from: shapes-stable reuses **622,173 of 663,134** shells, so nearly every one is the shell's OLD contents dying at `*b = map`. **The refcount arithmetic is the actual lever.** The caller cloned the plan (+1), the clone moved into the new map, and the shell's old keys — the SAME plan at a hot literal site — dropped (-1): two atomic RMWs per object to end where it started, plus a whole-struct move of fourteen fields. `alloc_finalized` now takes `&StaticKeyPlan`, and `refit_finalized` rebuilds the shell in place, keeping the `Arc` it already holds when `StaticKeyPlan::ptr_eq` says the plan is the same — **zero atomics in the matching case**, one clone plus one drop otherwise, exactly as before. Latch `ZIPP_NO_SHELL_REFIT`. **The safety property is in how it is written:** `refit_finalized` destructures `ObjMap` naming all fourteen fields with NO `..`, so adding a field breaks the build. That is load-bearing rather than stylistic — the pool admits a shell on `was_settled && Planned keys && empty store` and says nothing about `frozen`, `sealed`, `class` or `extensible`, so a field left holding the previous occupant's state would be a wrong answer, and the assignment form this replaces was safe precisely because it reset everything. |
| **B238 LANDED (the first PROFILE-GUIDED wave): the literal-born allocation path stops re-reading the object it just built** | **warm-router -2.61%, shapes-stable -2.06%, reactish-reconcile -1.75%, shapes-megamorphic -1.44%, allocation-survival -0.23% (21 alternating latch pairs each; outputs byte-identical on all five with the latch both ways)** | B237's profiler was pointed at the four biggest hostile rows and they returned the SAME top profile — `Heap::free_slot` 6.4-12.0%, `drop_in_place<ObjMap>` 4.3-7.5%, `Heap::alloc` 3.8-5.4%, `Heap::alloc_finalized` 3.6-6.6%, `Heap::refresh_mirror` 3.5-5.8% — so **one object-lifecycle fix moves four rows at once**, which is the finding that matters more than this wave. The cheapest piece first: `alloc` ends by calling `refresh_mirror`, which recovers a shape and a vals pointer by re-reading the occupant through its `Box` (a discriminant load plus two dependent loads) — values the finalize path had in registers, since `shape` is its own argument and the store pointer is in hand. Splitting slot acquisition (`alloc_slot`) from the mirror write lets `alloc_object_settled` write the mirror directly. The cold mirrors stay untouched, correct by the same B198 induction `refresh_mirror`'s `Object` arm relies on: an object owns none of them, and `free_slot` cleared whatever the previous occupant owned. Latch `ZIPP_NO_SETTLED_ALLOC`. **One measurement lesson worth keeping:** the first cut left `alloc_slot` at plain `#[inline]`, and with two callers LLVM stopped inlining it back into `alloc` — turning the GENERIC allocation path into a call it never used to make. survival, which is allocation-heavy but NOT literal-born, read **+0.73%** while the four literal-born rows read -1.0% to -1.5%. Forcing `#[inline(always)]` removed the regression AND improved all four (warm-router -0.99% -> -2.61%, shapes-stable -1.23% -> -2.06%). A refactor that only moves code can still cost, and the row that does not benefit is the one that detects it. |
| **B237 LANDED (CAPABILITY): the instruction-pointer profiler §6 has listed as missing since B3 — and the first profile it produced, which refutes two of this session's own charters** | no speed claim. **reactish-reconcile, 5,722 samples over 30 runs: `Heap::free_slot` 8.2%, `drop_in_place<ObjMap>` 4.5%, `mi_free_block_local` 4.3%, `Heap::alloc` 3.8%, `dispatch_body` 3.8%, `Heap::alloc_finalized` 3.6%, `Heap::refresh_mirror` 3.5% — allocation and freeing together ~43% of the row** | The phase profiler tags SUBSYSTEMS by hand, so a row that is 77.8% `jit-fast` is a dead end: it cannot say which code. Every hostile gap now lives inside that bucket, and this session burned three charters guessing at it (B231's argument-object elision, B232's key pooling, B235's `Object.keys` fast path). So: a second sampler mode, `ZIPP_PROF_PC=1`, that suspends the engine thread, reads `Rip`, resumes, and maps each sample onto the emitted body containing it. **No stack walking and no `StackWalk64`** — the objection recorded in `prof.rs` — because the JIT already knows the address range of every buffer it emits, so those ranges are registered at install time (whole-fn, all four region paths, and the map/reduce/filter kernels) and a sample is resolved by binary search. The suspend window contains two system calls and one `u64` read: the sample buffer is allocated once when the mode arms, so nothing allocates or takes a lock while the engine is stopped. For a sample that lands in `zipp.exe` itself, dbghelp resolves only the CRT — the release PDB carries public symbols only, and public-symbol lookup answers every address with the nearest preceding export, which reads as plausible nonsense (`_NLG_Return2` at 4.3%). So the profiler reports a module-relative OFFSET instead, `tools/pcmap.py` resolves offsets against an MSVC linker map, and a new `[profile.profiling]` (release + `debug`, nothing that affects codegen) provides the symbols. Windows x86-64 only; elsewhere the module is no-ops. **What the first profile settles:** the `jit-fast` share was misleading — only ~9% of samples are in registered emitted code and ~15% in emitted code overall, with the rest in the Rust engine, and ALLOCATION AND FREEING ARE ~43% OF REACTISH. That is the arena/nursery epic quantified at last, and it also explains why every peripheral charter measured null: nothing peripheral is where the time is. |
| **B236 MEASURED (no default changed): the mimalloc `secure` tax across ALL 17 HOSTILE ROWS — and it is NOT the binding constraint there** | **hostile geomean 0.9356 -> 0.8754 without hardening; rows above parity 9 -> 8. Exactly ONE row crosses on the policy alone: async-lived 1.007 -> 0.969** | B225 measured the tax on the REAL corpus and found it decisive there (json-large -36.1%, regex-log-scan -23.0%) — enough that both rows would cross. The open question was whether the same was true of the hostile rows, which are where the remaining gap lives. Method: three engines built from the same tree — `hard1` and `hard2` IDENTICAL (the shipped `secure` default) into separate target dirs so they link independently, `soft` with the `secure` feature dropped — run interleaved PER REPETITION, 9 reps, outputs compared for divergence on every case. **hard1-vs-hard2 is the A/A**, and it reads |+-1.3%| or better on every row above 30ms (its two larger readings, ephemeral +2.01% and calls-baseline +1.59%, are 13ms and 16ms rows). **Per-row tax:** survival -14.93%, reactish -12.62%, types-churn -12.48%, megamorphic -10.15%, shapes-stable -9.55%, async-burst -9.23%, warm-router -8.77%, ephemeral -5.32%, nanoid -4.09%, async-lived -3.76%, module-hot-graph -3.75%, types-stable -2.84%, branch-control -2.65%, bytecode-vm -2.58%, calls-closures -2.57%, throw-catch -2.36%, calls-baseline +0.12% (null). **The projection is the point.** Applying each row's own tax to the `0467393` capture: survival 1.901 -> 1.617, reactish 1.862 -> 1.627, warm-router 1.703 -> 1.554, shapes-stable 1.657 -> 1.499, megamorphic 1.604 -> 1.442, calls-closures 1.483 -> 1.445, nanoid 1.475 -> 1.415, types-churn 1.197 -> 1.048. Eight rows stay above parity, seven of them above 1.4x. So the hardening policy is worth a real 9-15% on the alloc-heavy hostile rows and **still leaves every one of them a long way from parity** — B225's real-corpus figures do not generalise, because json-large's allocation density is extreme even by this corpus's standards. **Consequence for the campaign: answering the hardening question buys json-large, regex-log-scan and async-lived, and nothing else. The rest of the hostile gap is engine work, and the census in B235 plus survival's telemetry say which kind — allocation throughput and sweep.** survival: `[concatmemo] hits=448968 misses=1032` (99.8% served, not a lever), and **GC 23.8-25.0ms = 10 minors at 12.6-13.3ms + 2 majors at 11.0-11.6ms** (three runs, idle machine), the minors sweeping 736,429 young slots at **~17.5ns each**. An earlier reading of 63.9ms/48ns was taken while `cargo test` was compiling on ten cores and is withdrawn — the counts held, the milliseconds did not, which is the session's own hygiene rule catching itself. At 17.5ns the sweep is not obviously mispriced, so the lever on this row is the ALLOCATION side and the major pair, not the minor sweep. The default remains `secure` — unchanged, as it has been at every checkpoint. |
| **B235 REFUTED BEFORE BEING BUILT — and the reactish-reconcile ALLOCATION CENSUS that refuted it** | no speed claim; the census is the deliverable | The plan was an `Object.keys` fast path: the generic arm re-reads every key's LIVE descriptor (a hash probe each) because a getter run for values/entries could have changed the map, and `Object.keys` runs no getter; it also runs `spec_key_order` on objects whose precomputed `has_element_key` bit already proves they have no index keys. Reading the bench first (the B233 lesson, applied): reactish calls `Object.keys` **25,200 times returning 33,600 keys in the whole run**, not the ~260k the call-site count suggests — `diff` bails at the `previous.key !== next.key` check for every item, because `children.reverse()` on odd ticks makes the keys mismatch, so the prop loop is never reached for the 28 items. The fast path would have measured null. NOT BUILT. **The census, from node counters plus zipp telemetry, is what the row actually costs:** 130,231 element objects + 117,628 prop objects + 117,628 argument objects + 122,325 patch objects (`[objpool] pushed=496730 popped=475188` — the pool already recycles essentially all of them), 117,628 arrow closures, and 172,241 string concats of which the B212 memo already serves **290,597 hits against 3,473 misses (98.8%)**. GC is **12.7-13.1ms over 64 minors, 12.0-12.4ms of it SWEEP** (three runs on an idle machine; an earlier reading of 21.3ms was taken while a test262 sweep was running and is withdrawn), against a row of ~85-103ms. So neither the enumeration path nor the concat path nor the object pool is the cost: the row is allocation THROUGHPUT plus sweep, i.e. the recorded nursery/arena epics, and — since B176 measured mimalloc `secure` at +28..57% precisely on alloc-heavy rows — substantially the hardening policy. **One bounded lever the census does expose:** 117,628 of the 496,730 pooled objects (23.7%) are the `Item({ id, active, revision, onSelect })` ARGUMENT objects, built only to be destructured by the callee's object-pattern parameter. That object has no name in the callee and cannot escape unless `arguments` is used, so a statically-known callee with an object-pattern parameter could take the fields directly and the literal would never exist. Worth roughly 5-6% of the row on these counts — real, but not a crossing on its own. |
| **B230 CHARTER (a systematic probe of every remaining hostile gap — recorded so it is never redone): the bounded ADMISSION wins are exhausted; what is left is CAPABILITY work** | no speed claim; this row exists to stop the next session re-deriving where the walls are | After capture #13, each remaining row was profiled and its decline census taken. **Profiles:** allocation-survival 68.5% jit-mem + 27.0% GC; reactish-reconcile 79.1% jit-fast + 12.9% GC; **calls-closures 98.0% jit-mem with ZERO gc and zero interp** — that row is now purely MEM-tier code quality, its call traffic already emitted; types-churn 77.6% jit-mem + 18.8% GC; npm-nanoid 77.6% jit-mem + 17.0% jit-fast with only 2.9% GC (profiled from the real bench dir — the scratchpad copy cannot resolve its sibling import), and its `[concatmemo]` reads 0 hits because it builds ids by `s += alphabet[i]`, an append the `StrAppendIndex` fusion already covers, not the `“prefix” + int` shape B212 serves. **INT-tier declines, every row:** `Call` — the register tier forbids calls outright, so the ONLY route onto it is `int_splice` FLATTENING the callee, which is exactly the B223 pattern that took parse-large-js −22.5%. **`int_splice` declines, all five MEM-bound rows:** “no leaf plan (not monomorphic / not inline-eligible)”. **And the leaf planner's own reason, which is the real wall:** calls-closures gives “same-proto leaf has captures or lexical this” nine times — its callees close over mutable cells and `build_leaf_inline_plan` requires `!lexical_this && upvalues.is_empty()`; types-churn and shapes-stable give “not leaf-eligible” (a large `switch` body, and an allocating one). So the next real lever is CAPTURE-AWARE leaf inlining — deep but well-defined, and B189's `upvals_mirror` already exposes the closure's upvalue base the spliced body would need. Alongside it the moving nursery (survival's 27% GC) and the `ObjMap` arena remain. None of these is another admission fix. |
| **B227 LANDED (the redirect paying off): admit MONOMORPHIC call sites to the fully-emitted cross3 lane** | **warm-router −4.91%, allocation-survival −2.97%, reactish-reconcile −2.77%; calls-closures and types-churn null; 5-row geomean −2.29% [−2.79,−1.90] — the broadest multi-row win of the session** | B222 and B226 both failed trying to make the cross-call HELPER cheaper, which redirected the epic to “stop taking the helper route at all”. Following that: `build_cross_call_plan` sets `same_proto` only in the `ic_call_same_proto` arm, and the cross3 gate requires it — so a plain MONOMORPHIC site, one filled way naming one callee and the most predictable call shape there is, was excluded from the emitted lane while the rotating-closure shape got it. Nothing in the lane depends on which arm supplied the exemplar: it bakes the callee's FID and revalidates per call against the hot mirror, so a mono exemplar is strictly the easier case. Evidence the diagnosis was right before the fix: a `ZIPP_NO_CROSS3` probe showed the lane already worth +12.9% on calls-closures but −0.9%/−0.8% (nothing) on reactish/types-churn — it simply was not engaging there. `ZIPP_NO_CROSS3_MONO=1` restores the old admission. **FOLLOW-UP, precisely located:** types-churn stays null because its callee has 94 registers, and cross3 declines `reg_count > 64` since its plan carries a `u64` mask — even though the JIT already computes a WIDE mask for that callee reading `marked=0`, i.e. nothing needs filling at all. Admitting the all-zero-wide-mask case would hand types-churn's 1.2M calls the lane, but it has a CORRECTNESS PREREQUISITE that must land first: `set_cross_entry` bumps `mask_gen` only when the INLINE mask changes, and for a wide callee that is always `u64::MAX`, so a wide-mask change would never bump the generation and a baked “nothing to fill” lane would keep skipping a fill that had become necessary. Fix the gen bump first, then widen. |
| **B226 REFUTED (built, measured, reverted) — and with B222 it REDIRECTS THE CROSS-CALL EPIC: the ~19ns floor is not made of shavable helper micro-costs** | **calls-closures +0.48%, reactish +0.57%, types-churn +0.37% — all slightly WORSE; null at best** | `Jit::cross_entry` hands back `(entry, mask, Option<JsonWalkPlan>, Option<MarkdownInlinePlan>)` BY VALUE, and `JsonWalkPlan` alone is ~56 bytes — so every cross call appeared to copy an ~88-byte tuple to use 16 bytes of it, with the plans consulted only in a rare `argc == 1` arm. Split it into a hot `cross_entry_fast` returning just `(entry, mask)` plus a cold `cross_entry_plans` fetched inside that arm. Measured null-to-negative: the accessor is `#[inline]`, so the compiler was ALREADY eliminating the unused fields, and the split merely added a second lookup on the cold path. REVERTED. **The redirect matters more than the result.** Two independent, well-motivated attacks on the cross-call enter cost have now come back null — B222 (the callee `FuncProto` read) and this one — and the window fill was already measured at ~2.7ns of ~19ns by the W7 census. That is strong evidence the residual is the IRREDUCIBLE work of the helper route itself (the callee heap read, the window/frame bookkeeping, the call sequence), not a pile of shavable checks. So the epic's real lever is NOT making the helper cheaper — it is widening the EMITTED cross3 lane's admission so the rows still on the helper route stop taking it at all. calls-closures, types-churn and reactish are exactly the rows cross3 declines today; that admission set, not the helper body, is where the next investigation belongs. |
| **B225 MEASUREMENT (no code change; the policy default is UNTOUCHED): what the hardened allocator actually costs, so the standing decision is a decision WITH NUMBERS** | **json-large −36.1% and regex-log-scan −23.0% without `mimalloc`'s `secure` feature; markdown-render −16.6%, map-set-heavy −15.3%. Applied to capture #12's ratios that is json-large 1.275× → ~0.82× and regex-log-scan 1.138× → ~0.88× — BOTH COMFORTABLY UNDER PARITY** | The campaign has carried “json/regex are entirely the hardening trade” since B182 with a recorded per-row range of 6–19%. That range is too low for these two rows, and the gap mattered because they are the only benches left whose parity is a POLICY question rather than an engineering one. Measured directly: built one binary with `features = ["secure"]` removed, restored the manifest in the same step (the default is deliberate and stays), and compared the two as symmetric scratchpad copies at 21 reps. Cross-checked in BOTH directions after this session's A/A lesson — forward reads −29.9% on the json+regex pair, reversed reads +44.4% (≡ −30.8%), so the effect is real and not the fresher-file artifact that faked a win in B224. WHAT THIS MEANS: the hardening turn is currently the sole reason two retained rows sit above Node, and unlike every other remaining gap it needs no engineering — it needs a call on whether the guarded-metadata / randomisation protection is worth ~36% on JSON-shaped and ~23% on regex-shaped allocation load. Recorded, not acted on: reverting a deliberate security posture is not a performance decision to make unattended. |
| **B224 CHARTER (scoped precisely; the emitted lane NOT attempted, and a cheaper substitute REFUTED — see the tail): the MEM-tier dense-Array `SetIndex` inline store** | est. shapes-stable −2.5..3%, allocation-survival ~−1%, reactish ~−0.4%, and the last bounded share of async-promise-chain's 1.08× — all AFTER this session's 5×-optimistic discount | The MEM tier gives `GetIndex` a pinned dense-array lane but sends every `SetIndex` to the generic `jit_set_index` helper (7.6ns/store measured, vs node's 0.6), and `region_mem.rs` filters the ARR kind out of `pinned` explicitly so the TypedArray store-encoding match can never see it. Two of the three stated blockers dissolve under an IN-BOUNDS guard, which the TypedArray lane already emits (`cmp rcx, [rsp+off+16]; jae => bail`): an in-bounds store cannot append and cannot realloc the `Vec`, so the pinned base stays valid and the region's staleness argument survives. The two that DO NOT dissolve, and which any attempt must handle explicitly: (1) **the GC write barrier** — unlike a TypedArray, an Array element is a `Value` that may be a heap reference, so an old-generation receiver needs the remembered-set/young-log barrier the helper performs today; (2) **hole-fill** — overwriting `Value::HOLE` is not a plain 8-byte store, and it is not an edge case to defer: `async-promise-chain`'s `partC` does `arr = new Array(WIDTH)` and then fills all WIDTH holes, so a lane that bails on holes wins EXACTLY NOTHING on the row that motivates it — which is what the lever panel's own judge warned. Also verified this session and worth not re-deriving: `partC`'s inner fill loop is NOT an INT-tier candidate (it stores promise objects; its `[7,18]` decline is correct), and types-churn has no concat headroom left — the B212 memo already serves it at a 95.2% hit rate, so its residue is the charted cross-call enter/close floor, not string work. **REFUTED SUBSTITUTE:** before attempting the lane, the obvious cheaper move was tried — `jit_set_index` consults `arr_props`, `array_length_nonwritable` and `proto_of` by HASH on every store (all three empty in any ordinary program, and a hole-fill pays all three), so `is_empty()` prefixes should have been free money. Measured NULL: async +0.13%, sparse-array +0.52%. An empty `FxHashMap` lookup is already near-free. **AND THE MEASUREMENT ITSELF NEARLY LIED:** the two-binary A/B read map-set-heavy at −6.07%, which would have been a false win — the A/A control (same source, built twice) reproduced −5.2% on that row, so the delta was the harness's first-side binary reading slow, not the change. LESSON, now standing: a two-binary A/B whose “old” side is a COPIED exe is contaminated (first-run paging/AV scan on the fresh file); prefer a latch, and never book a two-binary result without the A/A. **PRIORITY CORRECTION, from finishing the design:** the lane CANNOT cross any row, so it is not a path to the parity goal. Three facts settle that. (1) All four candidate rows fill `new Array(width)` — `batch[i]`, `fresh[i]`, `children[id]`, `arr[j]` — so EVERY store is a hole-fill and a hole-routing lane wins exactly zero; hole-fill is mandatory, not deferrable. (2) All four store HEAP values, so the GC barrier cannot be dodged by a non-heap-value guard; the sound instrument is a `gen[holder] == YOUNG` guard (the barrier is definitionally a no-op unless the holder is OLD, in both the ordinary and `valgrain` forms), and those arrays are freshly allocated per iteration so the guard would hit — but `gen` is NOT JIT-visible, so this needs a new raw base pointer carrying the same reallocation/refresh discipline as `versions`, i.e. a dangling-pointer risk class. (3) Even executed perfectly the judged ceiling is −2.5..3% on shapes-stable and ~−1% elsewhere, which after this session's demonstrated 5×-optimistic discount leaves stable at ~1.80×, survival ~1.88×, reactish ~1.88× and async ~1.07× — NO row crosses. Worth building for its own sake on a quiet day with the full battery; worthless as a parity move. The good news the design did establish: the pin snapshot already DECLINES for an `arr_props`/arguments array (all-zero slot, and a real heap Value's bits are never 0), so the identity guard the read lane uses handles that exotic for free — a store lane inherits it. |
| **B223 LANDED: the INT-splice replay allowlist admits a pinned-string `charCodeAt` — one call form proven pure, not admitted by name** | **parse-large-js −22.5% (343.9→266.6ms, 21 reps) — more than DOUBLE the −32ms floor the panel measured by hand-inlined twin; regex-log-scan and markdown-render null. Measured against node on a clean PGO build of `0162956`: parse-large-js **0.899×** (240.5ms zipp vs 267.4ms node) — UNDER PARITY, from 1.19×; polymorphic-objects holds at 0.985×. Second retained row crossed this session** | The int-splice flattens a monomorphic leaf into its caller, but declines when the span between the callee's `LoadGlobal` and the call contains a non-replayable op, because ANY guard exit inside that span resumes at its head and re-runs it. The allowlist is closed on purpose — `CallMethod` is exactly the shape that can run user code, and the admitted `arr.push(int)` would COMMIT an append and then replay it. parse-large-js's whole mix loop died on this: `h = mix(h, src.charCodeAt(starts[ti]))` puts a `CallMethod` in the span (`@298 non-replayable op @297`). Admitting it BY NAME would be unsound (a user object can own a `charCodeAt` with side effects), so the admission is by PROOF, three facts each checked in code: `Recv::Str` is produced only for a `CallMethod` whose name constant is literally “charCodeAt”; `STR_PIN_KIND` is assigned only when the RUNTIME receiver is a flat-ASCII `HeapObj::Str`, re-guarded per execution so a rope, a non-ASCII string or any other receiver deopts before the load; and the INT emitter's string-pin arm inlines a byte load that, in `plan_region`'s own admission comment, “runs no user code, allocates nothing”. Re-running it is therefore a pure re-read of the same byte. `pinned_arr_push` is explicitly NOT admitted. The win exceeds the twin's floor because the twin only measured inlining the callee, while the real splice lets the whole enclosing region reach the INT tier. `ZIPP_NO_SPLICE_CCA=1` restores the unconditional decline. |
| **B222 REFUTED (built, measured, reverted): the “thin cross-call descriptor” — caching the four callee fields the enter helper reads out of `FuncProto`** | **parse-large-js −0.72% (marginal), and types-churn +1.34% and markdown-render +0.94% WORSE; class-prototype-hot and reactish null. The lever hunt's −15..20ms estimate is refuted** | The retained-row panel priced parse-large-js's recursive-descent quartet at 1.41M boundary crossings × 19.6ns and proposed a thin monomorphic descriptor as one of three stacking levers. The reading was right about the traffic and wrong about the term: on every generic cross call the helper does one `self.func(fid)` to take `lexical_this`, `is_strict`, `reg_count` and `param_count` out of a large structure it touches for nothing else — which LOOKS like a cold indexed read per call. Built it properly (a packed u32 descriptor recorded beside the cross entry, `Jit::cross_desc`, cleared with the entry, `ZIPP_NO_CROSS_DESC` comparator, helper-side only so no emitted code changed) and measured it: the proto read is NOT a cost, because a hot call site crosses to the SAME few callees over and over and those cache lines stay warm — while the descriptor lookup adds an array bounds-check and a second cache line of its own. REVERTED. Third repricing of this panel's estimates (B219 was 5× optimistic, B220's first feed 4× under, this one negative): treat “N calls × measured-in-isolation ns” as an upper bound that assumes cold caches, never as a forecast. What remains real on that row is the INT-splice admission the panel measured by hand-inlined twin (−32ms floor), which is blocked on proving the span's `charCodeAt` is the pinned-string intrinsic rather than a user method — chartered, not attempted, because admitting it by NAME would double-execute a user call on every guard exit. |
| **B221 FINDING (method, not a wave): the async rows' PGO-retrain variance moves them ACROSS PARITY in both directions, so a single capture's reading of those rows is not a statement about the engine** | **`async-lived` measures 0.99× [0.97, 1.00] on a clean PGO build of `a230ab5` — 41ms zipp vs 42ms node — where capture #11 read it at 1.059×. The corpus scorecard is 12 rows above node, not 13** | Capture #11 showed `async-lived` crossing from 0.978× to 1.059× and `async-promise-chain` from 1.070× to 1.118× — both async rows moving together, which the campaign's method treats as a retrain signature rather than a mechanism. Bisected rather than assumed, on the capture's OWN binary and then on a fresh one: B218 measures −7.15% [−8.22,−6.38] on async-promise-chain (the row would read ~1.20× without it) and −0.45% on `lived`; B214 measures +0.18% and −0.64%. Neither wave regressed either row. The same `lived` binary that capture #11 timed at 45.9ms now reads 41.0–41.4ms after a retrain of identical source — a ~10% build-to-build swing on one row. RULE: for the async family, price mechanisms with one-binary latch A/Bs and treat capture-to-capture deltas as unreliable; never book a row move there as progress or regression without a covering latch. |
| **B220 LANDED: the sweep-AND-DELETE-fed key-buffer pool — the third member of the B194/B196b pool family** | **polymorphic-objects −2.08% (446.8→437.5ms, 21 reps); json-large and markdown-render null; 3-row geomean −0.68% [−1.0,−0.3]. With B219 the row is now **AT PARITY, measured directly on a clean PGO build of a230ab5 rather than compounded: 1.00× [0.98, 1.00], 322ms zipp vs 323ms node at 15 reps** — from capture #11's 1.045×. The first retained row this campaign has brought to parity by charted lever rather than by a favourable retrain** | Evidence first, because B219 had just repriced its own estimate 5×: a new `bench_append_decompose` micro prices the dynamic append's Rust side at **51.9ns owned / 33.2ns with a recycled buffer / 27.8ns on the no-alloc planned path**, so the per-append key malloc is 36% of it and a pool captures 78% of what the far more complex site-learned-plan adoption could. The pool: a dying object's OWNED key Strings are drained at `free_slot` (a `Planned` map's keys belong to a shared plan Arc and are not ours to take) and `obj["prefix" + i] = v` fills a recycled buffer instead of mallocing; bounded at 4096 entries × 64 bytes so a pathological key set cannot pin memory, and charged in `resident_bytes` (the B207 lesson). THE DIAGNOSTIC THAT MATTERED: the first build measured only −0.53%, and rather than accept or abandon it the new `[keypool]` oracle line said why — **served=450,560 missed=2,249,440**, a 16.7% serve rate. The sweep is a poor feed because objects do not die fast enough; DELETES are the good one, since `delete obj[k]` frees a buffer immediately and the very next append usually wants it. Routing `ObjMap::remove_at`'s freed String (it already owned it and simply dropped it) into the pool took the serve rate to **50.0%** and the row win to −2.08%, a 4× improvement from a one-line feed change. |
