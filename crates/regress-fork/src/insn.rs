...
pub enum Insn {
    ...
    /// `icase` is the effective ignore-case flag where the backreference
    /// appeared (modifier groups may scope it away from the global flag).
    BackRef {
        group: u32,
        icase: bool,
    },

    /// Perform a backreference match for a named group with multiple
    /// (duplicate-name) instances. At most one of `groups` can have
    /// participated in the match; match against that one, or the empty
    /// string when none participated.
    BackRefMulti {
        groups: Box<[u32]>,
        icase: bool,
    },
