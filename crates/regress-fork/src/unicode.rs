...
pub(crate) fn unicode_property_from_str(
    ...
    // Script=Unknown (alias Zzzz) is a valid property value (UTS #24): the code
    // points not assigned to any script (unassigned, surrogate, private-use).
    // The generated tables only enumerate assigned scripts, so use the
    // precomputed complement. Script_Extensions=Unknown is the same set.
    if matches!(
        name,
        Some(UnicodePropertyName::Script) | Some(UnicodePropertyName::ScriptExtensions)
    ) && matches!(s, "Unknown" | "Zzzz")
    {
        return Some(PropertyEscapeKind::CharacterClass(
            &crate::unicodetables_unknown::SCRIPT_UNKNOWN,
        ));
    }
    match name {
