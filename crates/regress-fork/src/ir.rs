...
pub enum Node {
    ...
    /// `icase` is the effective ignore-case flag where the assertion appears;
    /// it may differ from the global flag inside `(?i:...)` / `(?-i:...)`.
    ...
    /// not everything.
    /// `icase` is the effective ignore-case flag where the backreference appears;
    /// it may differ from the global flag inside `(?i:...)` / `(?-i:...)`.
    BackRef { group: u32, icase: bool },

    /// A backreference `\k<name>` where `name` is shared by multiple capture
    /// groups (necessarily in distinct alternatives). At most one of `groups`
    /// can have participated in the match; the backreference matches that
    /// group's text, or the empty string when none participated.
    /// Indices are 1-based, like `BackRef`.
