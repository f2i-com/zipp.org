        ...
        match to_char_sat(c) {
            ...
            'p' | 'P' if self.flags.unicode || self.flags.unicode_sets => {
                ...
                match property_escape {
                    PropertyEscapeKind::CharacterClass(s) => {
                        ...
                        if self.flags.icase {
                            // Close the set under case folding (ES2025 22.2.2.9
                            // CharacterSetMatcher canonicalizes the input). In unicode
                            // mode the \P complement applies before the case closure:
                            // fold('A') = 'a' is in the complement of Lu, so
                            // /(?i)\P{Lu}/u matches 'A'. In unicode-sets mode case
                            // folding applies first (MaybeSimpleCaseFolding), so the
                            // complement of the closed set is wanted; keep `invert`.
                            if invert && !self.flags.unicode_sets {
                                cps = cps.inverted();
                                invert = false;
                            }
            ...
            'k' if self.flags.unicode || !self.named_group_indices.is_empty() => {
                ...
                let node = match group_indices.len() {
                    ...
                    _ => {
                        // Unusual case of multiple groups sharing a name (necessarily in
                        // distinct alternatives): the backref matches the text of whichever
                        // group participated, or the empty string when none did. Note an
                        // alternation of backreferences is NOT correct here: the first
