...
| Check | Result | Scope |
| --- | --- | --- |
| Node host and session tests | passed, 0 failed | Loader, manifests, checkpoint/tokenizer refusal, binary bindings, bounds, hashing, lifecycle; session lifecycle uses explicit doubles. The whole suite, of which the GGUF and Qwen3 rows below are a part. The suites that need a real checkpoint (`ZIPP_GGUF_MODEL`, `ZIPP_QWEN3_MODEL`, `ZIPP_GPT_NEO_MODEL`, `ZIPP_TRANSFORMERS_MODEL`) skip without one, since none is redistributed here; everything else runs against the bundled tiny Qwen3, and under `ZIPP_REQUIRE_INTEGRATION=1`, as CI runs it, a missing engine fails rather than skips. How many tests that is changes with every commit, so it is not recorded here: `npm test` prints it |
...
| Recurrent primitives on the runtime | passed | A causal depthwise convolution over a carried window and a gated delta-rule state update, each built from existing operations and checked against an independent implementation |
| CPython/NumPy plugin tests | passed | 12 for the bundled fixtures, 15 for GPT-Neo: config reading, logit parity, greedy parity, checkpoint-order tensors, local-attention windows, tanh GELU, unscaled attention, family refusals, and 27 differential tokenizer cases against transformers |
...
| Quantized matmul on the backends Node can reach (`gpu-lab/tests/quantized.test.mjs`) | passed | `cpu-js` and `wasm` hold a Q4_K/Q6_K matmul bit-for-bit equal to the same matmul over decoded values, and hold the decoder against `f2i-gguf-quants` rather than against a second copy of itself |
...
| GGUF reader (`tests/gguf.test.mjs`) | passed | Header parsing, metadata, tensor ranges in this package's shape order, row gathering without decoding the table, budget refusals, malformed-shape refusal |
| GGUF module detection and provenance (`tests/gguf-module.test.mjs`) | passed | Absent module reported rather than thrown, wrong module refused by shape, and the committed tree checked against every digest `scripts/fetch_gguf_wasm.sh` recorded — including that `rust/Cargo.toml` pins the revision the lock names. Which release that is, `wasm/gguf-wasm.lock.json` says; naming it here as well would be one more copy of a version number to go stale, which is the thing the lock exists to stop |
| GGUF tokenizer (`tests/tokenizer.test.mjs`) | passed | Built inside the module from the file's own vocabulary and `tokenizer.ggml.pre`; qwen2 digit-splitting checked against the vocabulary's own rule, and an unimplemented pre-tokenizer name refused rather than guessed |
| Qwen3-0.6B Q4_K_M end to end (`tests/qwen3.test.mjs`) | passed | Configuration read from the file; prompt tokenized to the ids the vocabulary defines; **373 MB resident, 372 MB of it still blocks**; a prefill of "The capital of France is" predicting ` Paris`; and a cached decode agreeing with the prefill before generating |
...
| `gpu-lab/wasm/kernels.wasm` reproduces | passed, and gated in CI | Built on Windows and on Linux, from different directories with different `CARGO_HOME`s: identical bytes. The build remaps every recorded path to a fixed, slash-separated form and strips symbol names -- whose hashes otherwise encode the workspace's location -- so the committed binary records neither who built it nor where. The `python` CI job rebuilds it with `--locked` and fails on any difference from what is committed |
| Qwen3 split across stages (`tests/qwen3-stages.test.mjs`) | passed, logits identical bit for bit | Layers 0–13 on one runtime, 14–27 on another, the residual stream between them, compared with the same model in one graph at every position of the prompt. Equality rather than a tolerance: the same kernels over the same weights in the same order, so nothing in the seam should compute. The hidden state is round-tripped through a structured clone first, which copies the buffer the way `postMessage` does — 4,096 bytes a token, while 372 MB of weights never move. Each stage is bound only its own tensors, and carries only its own caches — two a layer, checked to be its own layers. Also in four stages of seven, for both prefill and decode: a prompt through four graphs gives the whole model's logits, and seven decode positions in sequence do too, which is where a cache written or read wrongly would show. |
| Stages across worker threads (`tests/qwen3-workers.test.mjs`) | passed | Three workers holding ten, nine and nine layers, hidden states **transferred** rather than copied — the sender is asserted to be left with a detached buffer, which in-process staging never exercised. A terminated stage produces an error naming it rather than a hang, every round-trip being bounded. And restarting one is measured not to be recovery: a replacement with empty caches differs in **151,936 of 151,936 logits** and predicts " the" where the model says " Paris", so a failed peer means re-prefilling its range |
| A stage refuses a hidden state that is not its predecessor's (`tests/qwen3-stages.test.mjs`) | passed | Refused by manifest before anything is computed: a different configuration of the same width, a gap or an overlap in the layers, and -- the case a configuration cannot see -- the same configuration with other weights. That one is the fixture with a single bit of one head-stage weight changed: its configuration digest is identical, and the sha256 of the file each host opened, which `prepareDecode({checkpoint})` attaches to the stage's manifest as `checkpoint_digest`, is not. A predecessor that leaves the digest out is refused as well, and so is a plan prepared without one being handed a digest it cannot compare; single steps and chunks alike |
| A float16 seam, if one were adopted (`tests/qwen3-half-seam.test.mjs`) | measured, not adopted | Halving what crosses between peers costs 1.9e-3 to 4.1e-3 over one seam and 2.8e-3 to 6.0e-3 over three, against both the float32 whole model and transformers — the two agree, since the narrowing dominates the 7.4e-5 baseline by about eighty times. Over three prompts the argmax never moved and the top five never reordered. Error grows with seams but sublinearly: three cost about 1.5× one, not 3×. The conversion itself is checked first, including every one of the 63,488 finite half values round-tripping. Nothing is narrowed in the product: the seam is float32 and bit-for-bit, and this is what the option would cost |
...
afterwards, so the architecture itself did not move. (That refusal is history:
`90e0a9c7` taught the frontend to resolve relative imports as CPython does. The
plugins kept their absolute imports, which work either way.)
