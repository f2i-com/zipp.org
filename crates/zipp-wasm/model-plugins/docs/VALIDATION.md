...
| Block decoding vs an independent implementation | 11 of 15 formats, bit for bit | The decoders ZIPP reads quantized weights with are checked against [gguf-py](https://github.com/ggml-org/llama.cpp/tree/master/gguf-py), the llama.cpp project's own numpy GGUF library, in `gguf-quants/tests/golden/`. This is the piece that was missing: the quantized-vs-decoded tests held both sides to the *same* decoder, so a decoder wrong in one place would have agreed with itself. Q2_K, Q3_K, IQ4_NL and IQ4_XS have no vectors, needing a checkpoint quantized that way, and a test names them |
| Qwen3 quantized logits vs an external reference, end to end | NOT RUN | Three links of the chain are each checked — the architecture against transformers, the quantized execution against the decoded execution bit-for-bit, and now the decoder against gguf-py — but nothing yet runs this exact GGUF file through an independent implementation and compares final logits. The composition is strongly implied rather than measured, and closing it means an oracle that reads GGUF *and* runs the model, which is llama.cpp or a numpy reimplementation |
