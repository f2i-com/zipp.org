 ...
 */
function dequantize(dtype, bytes, elements) {
 ...
 * @returns {boolean}
 */
function header_needs_more_bytes(head) {
    const ptr0 = passArray8ToWasm0(head, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.header_needs_more_bytes(ptr0, len0);
    return ret !== 0;
}
exports.header_needs_more_bytes = header_needs_more_bytes;

/**
 * The dtypes this build decodes, so a host can say so before it starts.
 * @returns {any[]}
 */
function supported_dtypes() {
    const ret = wasm.supported_dtypes();
    var v1 = getArrayJsValueFromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 4, 4);
    return v1;
}
exports.supported_dtypes = supported_dtypes;
