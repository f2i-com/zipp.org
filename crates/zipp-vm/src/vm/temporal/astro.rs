...
fn ephemeris_correction(t: f64) -> f64 {
    ...
    match year {
        ...
        1986..=2005 => {
            poly(y, &[63.86, 0.3345, -0.060_374, 0.001_727_5, 0.000_651_814, 0.000_023_735_99])
                / 86400.0
        }
        1900..=1985 => poly(
            c,
        ...
        1700..=1799 => {
            poly((year - 1700) as f64, &[8.118_780_842, -0.005_092_142, 0.003_336_121, -0.000_026_6484])
                / 86400.0
        }
        1600..=1699 => {
            poly((year - 1600) as f64, &[120.0, -0.9808, -0.015_32, 1.0 / 7129.0]) / 86400.0
        }
        500..=1599 => poly(
            (year - 1000) as f64 / 100.0,
            &[1574.2, -556.01, 71.234_72, 0.319_781, -0.850_346_3, -0.005_050_998, 0.008_357_2073],
        ) / 86400.0,
        -499..=499 => poly(
            year as f64 / 100.0,
            &[10583.6, -1014.41, 33.783_11, -5.952_053, -0.179_845_2, 0.022_174_192, 0.009_031_6521],
        ) / 86400.0,
        _ => {
...
fn mean_new_moon(n: f64) -> f64 {
    ...
    J2000
        + poly(
            c,
            &[5.097_66, MEAN_SYNODIC_MONTH * 1236.85, 0.000_154_37, -0.000_000_15, 0.000_000_000_73],
        )
}
...
pub(crate) fn nth_new_moon(n: i64) -> f64 {
    ...
    let solar_anomaly = poly(c, &[2.5534, 1236.85 * 29.105_356_7, -0.000_001_4, -0.000_000_11]);
    let lunar_anomaly = poly(
        c,
        &[201.5643, 385.816_935_28 * 1236.85, 0.010_758_2, 0.000_012_38, -0.000_000_058],
    );
    let moon_argument = poly(
        c,
        &[160.7108, 390.670_502_84 * 1236.85, -0.001_611_8, -0.000_002_27, 0.000_000_011],
    );
    let omega = poly(c, &[124.7746, -1.563_755_88 * 1236.85, 0.002_067_2, 0.000_002_15]);
...
mod tests {
    ...
    fn rd(y: i64, m: i64, d: i64) -> f64 {
        ...
        let yy = y + 4800 - a;
        let mm = m + 12 * a - 3;
        (d + (153 * mm + 2) / 5 + 365 * yy + yy / 4 - yy / 100 + yy / 400 - 32045 - 1_721_425) as f64
    }
    ...
    fn gregorian_year_boundaries() {
        for (y, m, d) in [
            (1900, 1, 1), (1900, 12, 31), (1929, 1, 1), (1970, 1, 1), (2000, 2, 29),
            (2100, 12, 31), (1, 1, 1), (0, 12, 31), (-1, 1, 1), (-4713, 1, 1),
        ] {
    ...
    fn solar_longitude_matches_published_equinoxes() {
        ...
        let cases = [
            (0.0, rd(2024, 3, 20) + (3.0 * 3600.0 + 6.0 * 60.0 + 21.0) / 86400.0),
            (90.0, rd(2024, 6, 20) + (20.0 * 3600.0 + 50.0 * 60.0 + 56.0) / 86400.0),
            (180.0, rd(2024, 9, 22) + (12.0 * 3600.0 + 43.0 * 60.0 + 36.0) / 86400.0),
            (270.0, rd(2024, 12, 21) + (9.0 * 3600.0 + 20.0 * 60.0 + 34.0) / 86400.0),
            (0.0, rd(2000, 3, 20) + (7.0 * 3600.0 + 35.0 * 60.0 + 15.0) / 86400.0),
            (270.0, rd(2000, 12, 21) + (13.0 * 3600.0 + 37.0 * 60.0 + 30.0) / 86400.0),
        ];
    ...
    fn new_moon_matches_published_instants() {
        ...
        for published in cases {
            ...
            assert!(err < 90.0, "new moon at R.D. {published}: off by {err:.1} s");
        }
    }
    ...
    fn new_moon_sequence_is_monotone_and_invertible() {
        for &(lo, hi) in &[(-16_000i64, 13_000i64), (-3_100_000, -3_099_900), (3_099_900, 3_100_000)]
        {
    ...
    fn new_moon_series_correction_stays_small() {
        ...
        for n in -800..12_500 {
            let dev = nth_new_moon(n) + ephemeris_correction(nth_new_moon(n))
                - mean_new_moon(n as f64);
            assert!(dev.abs() < 0.9, "lunation {n} deviates {dev} d from the mean");
        }
    }
}
