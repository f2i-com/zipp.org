...
impl<'p> Vm<'p> {
    ...
    pub(crate) fn build_function_kind(&mut self, args: &[Value], kind: u8) -> Result<Value, Thrown> {
        ...
        self.do_eval(&source, false, false, None, None, false, false, Value::UNDEFINED, None, false, None)
    }
    ...
    pub(crate) fn shadowrealm_op(&mut self, op: u16, this: Value, args: &[Value]) -> Result<Value, Thrown> {
        ...
        match op {
            native::SHADOWREALM_EVALUATE => {
                ...
                let result = match self.do_eval(&code, false, false, None, None, false, false, Value::UNDEFINED, None, false, None) {
                    Ok(r) => r,
                    Err(_) => {
                        return Err(Thrown(
