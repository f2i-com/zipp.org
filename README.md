...
**Conformance — 99.82% of test262**, 95,678 of 95,848 required executions
...
|---|---|---|
| ECMA-262 + staging, both modes | 95,848 | 95,678 (99.82%) |
| intl402 (opt-in, `--include-intl402`) | 6,682 | 5,702 (85.3%) |

That is up from 96.97% under `oxc_parser`, and the increase is the whole reason
the engine grew its own front end.

170 executions still fail. The two largest items are known and bounded:
...
`chinese` and `dangi` still raise RangeError rather than approximating: they
need true new-moon instants and solar-term positions, which are not
closed-form.
